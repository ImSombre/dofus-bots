...
class CombatEngine:
    ...
    def on_mp_detected(self):
        ...
        webhook = self.config.data.get("discord_webhook", "")
        if webhook:
            message = "🚨 **ALERTE MP DOFUS**\n\n"
            message += "Un message privé a été reçu!\n"
            message += f"⏱️ {datetime.now().strftime('%H:%M:%S')}\n"
            message += f"⚔️ Combats effectués: {self.stats.get('combats', 0)}"
            
            if send_discord(webhook, message):
    ...
    def replay_actions(self):
        """Rejoue les actions enregistrées avec les VRAIS délais"""
        ...
        self.log(f"▶️ Replay de {len(actions)} actions...")
        
        # Calculer la durée totale
        if actions:
            total_duration = actions[-1]["time"]
            self.log(f"   Durée totale: {total_duration:.1f}s")
        ...
        for i, action in enumerate(actions):
            ...
            # Attendre le bon moment (timing exact)
        ...
        replay_duration = time.time() - start_replay
        self.log(f"✅ Replay terminé ({replay_duration:.1f}s)")
    ...
    def run(self):
        ...
        while self.running:
            ...
            try:
                ...
                # Vérifier les MP (toutes les 10 itérations pour pas ralentir)
                mp_check_counter += 1
                if mp_check_counter >= 10:
                    mp_check_counter = 0
...
class CombatGUI:
    ...
    def create_widgets(self):
        ...
        tk.Label(row1, text="Délai recherche mob:", font=('Segoe UI', 9),
        ...
        tk.Label(row2, text="Délai entre actions:", font=('Segoe UI', 9),
                bg=self.colors['bg2'], fg=self.colors['text2']).pack(side='left')
        ...
        # Boutons MP
        mp_btn_frame = tk.Frame(left, bg=self.colors['bg2'])
        mp_btn_frame.pack(fill='x', padx=10, pady=5)
        
        tk.Button(mp_btn_frame, text="📸 Capturer MP", font=('Segoe UI', 9),
                 bg=self.colors['warning'], fg='white',
                 command=self.capture_mp_template, cursor='hand2').pack(side='left', padx=5)
        
        tk.Button(mp_btn_frame, text="📱 Discord", font=('Segoe UI', 9),
                 bg='#5865F2', fg='white',
        ...
        self.mp_status_label = tk.Label(left, text=mp_status, font=('Segoe UI', 9),
                                        bg=self.colors['bg2'], fg=self.colors['text2'])
        self.mp_status_label.pack(pady=2)
        
        # Status webhook
        webhook_status = "✅ Discord configuré" if self.config.data.get("discord_webhook") else "❌ Discord non configuré"
        self.webhook_label = tk.Label(left, text=webhook_status, font=('Segoe UI', 9),
                                      bg=self.colors['bg2'], fg=self.colors['text2'])
    ...
    def bot_callback(self, event, data):
        ...
        if event == "log":
            ...
            self.root.after(0, self.on_mp_detected)
    
    def on_mp_detected(self):
        """Appelé quand un MP est détecté"""
        self.status_label.config(text="🚨 MP DÉTECTÉ!", fg=self.colors['record'])
        self.start_btn.config(state='normal')
        self.record_btn.config(state='normal')
        self.pause_btn.config(state='disabled', text="⏸️ PAUSE")
        self.stop_btn.config(state='disabled')
        
        messagebox.showwarning("MP Détecté!", 
    ...
    def toggle_mp_detection(self):
        """Active/désactive la détection MP"""
    ...
    def _do_capture_mp(self):
        ...
        self.log("📸 Place ta souris sur le texte MP...")
        time.sleep(1)
        self.log("⏳ 2...")
        time.sleep(1)
        self.log("⏳ 1...")
        ...
        if template.size > 0:
            ...
            self.root.after(0, lambda: self.mp_status_label.config(text="✅ Template MP configuré"))
        else:
            self.log("❌ Erreur capture")
    ...
    def open_webhook_config(self):
        ...
        def save_webhook():
            ...
            status = "✅ Discord configuré" if url else "❌ Discord non configuré"
            self.webhook_label.config(text=status)
            
            self.log("✅ Webhook sauvegardé!")
            dialog.destroy()
